...
AguasPerm<- readOGR("C:/Users/Usuario/Downloads/1AGUARATESINA/Usosdesuelo/Cuerpos_de_Agua_prov", "cuerpos_de_agua_prov_perm")
plot(AguasPerm)
head(AguasPerm@data)
...
proj4string(AguasPerm_sptr)

plot(AguasPerm_sptr)
plot (sta_fe_sptr, add = TRUE)
extent(sta_fe_sptr)
extent(AguasPerm_sptr)

r<-raster(sta_fe_sptr)
r
res(r) <- 500
AguasPerm_sptr_raster <- rasterize(AguasPerm_sptr, r, field = "TIPO")
plot(AguasPerm_sptr_raster)
AguasPerm_sptr_raster[AguasPerm_sptr_raster==2] <- 1
...
distAgPerm <- distance(AguasPerm_sptr_raster)
plot(distAgPerm)
plot (sta_fe_sptr, add = TRUE)

distAgPerm_crop <- crop(distAgPerm, sta_fe_sptr)
